...
int RawLogger::encode_frame(const uint8_t *y_ptr, const uint8_t *u_ptr, const uint8_t *v_ptr,
  ...
  int err = avcodec_send_frame(codec_ctx, frame);  // Send the frame to the encoder
  if (err < 0) {
    LOGE("sending frame to encoder failed\n");
    ret = -1;
  } else {
    err = avcodec_receive_packet(codec_ctx, &pkt);  // Receive the encoded packet
    if (err < 0) {
      LOGE("receiving packet from encoder failed\n");
      ret = -1;
    } else {
      av_packet_rescale_ts(&pkt, codec_ctx->time_base, stream->time_base);
      pkt.stream_index = 0;
